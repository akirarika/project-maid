...
[x] Git 功能：Commit 模板功能，和约束 Commit 格式
...
- `/.pm/templates/你的模板名称` 内部的所有文件夹的名称，会尝试使用模板变量。

- `/.pm/templates/你的模板名称` 内部的所有以 `.tpl` 结尾的文件，他们的文件名称及文件中的内容，会尝试使用模板变量。

- `/.pm/templates/你的模板名称` 内部的不以 `.tpl` 结尾的文件，将被**原封不动的复制**。

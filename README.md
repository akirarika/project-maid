...
在此目录中，我们再新建一个文件夹，名为 `{{your-name}}-view`。是的， **这是一个包含大括号的文件名！** 大括号和其内容，未来会由 Project Maid， **自动替换为我们想要的名称~** 我们先如实地复制这个文件名称即可。
...
尽管，前文所述的两种方法已经可以满足大部分场景，但有时，我们还需要更加可自定义的方式完成我们想要的操作。例如，当我们创建一个模型文件后，我们需要运行框架的命令，实现自动从数据库中查询此模型对应的表中所拥有的列，同时更新到我们新创建的模型文件中。
...
- 推送到远程仓库的指定分支，如果分支不存在，自动帮你创建

- 结束后，自动帮你切换回原先的当前分支

- 如果你选择的分支不是当前分支，还会同时帮你推送到当前分支

只需要点击右上角的 Github 图标即可 (章鱼猫猫)。
